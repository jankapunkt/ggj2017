...
public class MapPart extends GameModelInstance implements ICollidable {
    ...
    public MapPart(Model model, Vector3 position, btRigidBody.btRigidBodyConstructionInfo bodyConstructionInfo) {
    ...
    public MapPart(Model model, Vector3 position, float mass) {
	super(model, position);
	calculateBoundingBox(bounds);
	System.out.println("create new map part");
	colShape = new btBoxShape(new Vector3(bounds.getWidth() / 2, bounds.getHeight() / 2, bounds.getDepth() / 2));
    ...
    public MapPart(Model model, Vector3 position) {
	super(model);
